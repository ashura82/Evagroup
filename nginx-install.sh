...
if [[ $pagespeed == 'y' ]];then 
	NGINX_NTM=$(echo $NGINX_NTM;echo "--add-dynamic-module=/usr/local/src/nginx/modules/incubator-pagespeed-ngx-${PAGESPEED_VER}-stable")
...
./configure $NGINX_NTM >> /tmp/nginx-autoinstall.log 2>&1
...
if [[ ($rc2 != 0) && ($rc3 != 0) ]]; then
	ln -s -d /etc/nginx/sites-enabled /etc/nginx/sites-available
fi
